...
namespace HutongGames.PlayMaker.Actions
{
    ...
    [Tooltip("Performs a DELETE request to a REST API endpoint with full configuration options.")]
    public class APIDelete : FsmStateAction
    {
        ...
        public override void Reset()
        {
            ...
            userAgent = "Unity PlayMaker API Delete";
            timeout = 30f;
        ...
        public override void OnEnter()
        {
            ...
            retryCount = 0;
            Fsm.Owner.StartCoroutine(SendDeleteRequest());
        }
        ...
        private IEnumerator SendDeleteRequest()
        {
            ...
            {
                Debug.Log($"[API DELETE] Request to: {finalUrl}");
            }
        ...
        private void HandleResponse(UnityWebRequest request)
        {
            ...
            {
                Debug.Log($"[API DELETE] Response {code}: {response}");
            }
        ...
        private void HandleTimeout(UnityWebRequest request)
        {
            ...
            if (logResponse.Value || debugMode.Value)
            {
                Debug.LogWarning("[API DELETE] Request timeout");
            }
            ...
            if (retryCount < maxRetries.Value)
            {
                ...
                if (debugMode.Value)
                {
                    Debug.Log($"[API DELETE] Retrying... Attempt {retryCount}/{maxRetries.Value}");
                }
        ...
        private void HandleNetworkError(UnityWebRequest request)
        {
            ...
            if (logResponse.Value || debugMode.Value)
            {
                Debug.LogError($"[API DELETE] Network Error: {request.error}");
            }
        ...
        private void HandleOtherError(UnityWebRequest request)
        {
            ...
            if (logResponse.Value || debugMode.Value)
            {
                Debug.LogError($"[API DELETE] Error: {request.error}");
            }
        ...
        private IEnumerator RetryRequest()
        {
            ...
            yield return SendDeleteRequest();
        }
        ...
        private void ParseJSON(string json)
        {
            ...
            {
                Debug.LogError($"[API DELETE] JSON Parse Error: {e.Message}");
            }
        }
        ...
        {
            Debug.Log("=== API DELETE Request Details ===");
