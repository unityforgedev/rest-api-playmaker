...
namespace HutongGames.PlayMaker.Actions
{
    ...
    [Tooltip("Performs a PUT request to a REST API endpoint with full configuration options.")]
    public class APIPut : FsmStateAction
    {
        ...
        public override void Reset()
        {
            ...
            userAgent = "Unity PlayMaker API Put";
            timeout = 30f;
        ...
        public override void OnEnter()
        {
            ...
            retryCount = 0;
            Fsm.Owner.StartCoroutine(SendPutRequest());
        }
        ...
        private IEnumerator SendPutRequest()
        {
            ...
            {
                Debug.Log($"[API PUT] Request to: {finalUrl}");
                Debug.Log($"[API PUT] Body: {requestBody.Value}");
            }

            UnityWebRequest request = UnityWebRequest.Put(finalUrl, requestBody.Value ?? "");
        ...
        private void HandleResponse(UnityWebRequest request)
        {
            ...
            {
                Debug.Log($"[API PUT] Response {code}: {response}");
            }
        ...
        private void HandleTimeout(UnityWebRequest request)
        {
            ...
            if (logResponse.Value || debugMode.Value)
            {
                Debug.LogWarning("[API PUT] Request timeout");
            }
            ...
            if (retryCount < maxRetries.Value)
            {
                ...
                if (debugMode.Value)
                {
                    Debug.Log($"[API PUT] Retrying... Attempt {retryCount}/{maxRetries.Value}");
                }
        ...
        private void HandleNetworkError(UnityWebRequest request)
        {
            ...
            if (logResponse.Value || debugMode.Value)
            {
                Debug.LogError($"[API PUT] Network Error: {request.error}");
            }
        ...
        private void HandleOtherError(UnityWebRequest request)
        {
            ...
            if (logResponse.Value || debugMode.Value)
            {
                Debug.LogError($"[API PUT] Error: {request.error}");
            }
        ...
        private void ParseJSON(string json)
        {
            ...
            {
                Debug.LogError($"[API PUT] JSON Parse Error: {e.Message}");
            }
        }
        ...
        {
            Debug.Log("=== API PUT Request Details ===");
