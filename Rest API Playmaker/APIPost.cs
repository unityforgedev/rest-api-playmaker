...
namespace HutongGames.PlayMaker.Actions
{
    ...
    [Tooltip("Performs a POST request to a REST API endpoint with full configuration options.")]
    public class APIPost : FsmStateAction
    {
        ...
        public override void Reset()
        {
            ...
            userAgent = "Unity PlayMaker API Post";
            timeout = 30f;
        ...
        public override void OnEnter()
        {
            ...
            retryCount = 0;
            Fsm.Owner.StartCoroutine(SendPostRequest());
        }
        ...
        private IEnumerator SendPostRequest()
        {
            ...
            {
                Debug.Log($"[API POST] Request to: {finalUrl}");
                Debug.Log($"[API POST] Body: {requestBody.Value}");
            }
        ...
        private void HandleResponse(UnityWebRequest request)
        {
            ...
            {
                Debug.Log($"[API POST] Response {code}: {response}");
            }
        ...
        private void HandleTimeout(UnityWebRequest request)
        {
            ...
            if (logResponse.Value || debugMode.Value)
            {
                Debug.LogWarning("[API POST] Request timeout");
            }
            ...
            if (retryCount < maxRetries.Value)
            {
                ...
                if (debugMode.Value)
                {
                    Debug.Log($"[API POST] Retrying... Attempt {retryCount}/{maxRetries.Value}");
                }
        ...
        private void HandleNetworkError(UnityWebRequest request)
        {
            ...
            if (logResponse.Value || debugMode.Value)
            {
                Debug.LogError($"[API POST] Network Error: {request.error}");
            }
        ...
        private void ParseJSON(string json)
        {
            ...
            {
                Debug.LogError($"[API POST] JSON Parse Error: {e.Message}");
            }
        }
        ...
        {
            Debug.Log("=== API POST Request Details ===");
