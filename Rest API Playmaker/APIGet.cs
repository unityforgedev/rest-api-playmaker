...
namespace HutongGames.PlayMaker.Actions
{
    ...
    [Tooltip("Performs a GET request to a REST API endpoint with full configuration options.")]
    public class APIGet : FsmStateAction
    {
        [RequiredField]
        ...
        public override void Reset()
        {
            ...
            userAgent = "Unity PlayMaker API Get";
            timeout = 30f;
        ...
        public override void OnEnter()
        {
            ...
            retryCount = 0;
            Fsm.Owner.StartCoroutine(SendGetRequest());
        }
        ...
        private IEnumerator SendGetRequest()
        {
            ...
            {
                Debug.Log($"[API GET] Request to: {finalUrl}");
            }
        ...
        private void HandleTimeout(UnityWebRequest request)
        {
            ...
            if (logResponse.Value || debugMode.Value)
            {
                Debug.LogWarning("[API GET] Request timeout");
            }
            ...
            if (retryCount < maxRetries.Value)
            {
                ...
                if (debugMode.Value)
                {
                    Debug.Log($"[API GET] Retrying... Attempt {retryCount}/{maxRetries.Value}");
                }
        ...
        private void HandleNetworkError(UnityWebRequest request)
        {
            ...
            if (logResponse.Value || debugMode.Value)
            {
                Debug.LogError($"[API GET] Network Error: {request.error}");
            }
        ...
        private void ParseJSON(string json)
        {
            ...
            {
                Debug.LogError($"[API GET] JSON Parse Error: {e.Message}");
            }
        }

        private string ExtractJSONValue(string json, string path)
        {
            // Basic JSON path extraction (supports simple dot notation)
        ...
        {
            Debug.Log("=== API GET Request Details ===");
