...
namespace HutongGames.PlayMaker.Actions
{
    ...
    [Tooltip("Performs a HEAD request to a REST API endpoint. Returns only headers without body content.")]
    public class APIHead : FsmStateAction
    {
        ...
        [Tooltip("Store Content-Type header value")]
        [UIHint(UIHint.Variable)]
        public FsmString contentType;

        [Tooltip("Store Content-Length header value")]
        [UIHint(UIHint.Variable)]
        public FsmString contentLength;

        [Tooltip("Store Last-Modified header value")]
        [UIHint(UIHint.Variable)]
        public FsmString lastModified;

        [Tooltip("Store ETag header value")]
        [UIHint(UIHint.Variable)]
        public FsmString eTag;
        ...
        public override void Reset()
        {
            ...
            userAgent = "Unity PlayMaker API Head";
            timeout = 30f;
        ...
        public override void OnEnter()
        {
            ...
            retryCount = 0;
            Fsm.Owner.StartCoroutine(SendHeadRequest());
        }

        private IEnumerator SendHeadRequest()
        {
            string finalUrl = BuildUrl();

            if (logRequest.Value || debugMode.Value)
            {
                Debug.Log($"[API HEAD] Request to: {finalUrl}");
            }

            UnityWebRequest request = UnityWebRequest.Head(finalUrl);
        ...
        private void HandleResponse(UnityWebRequest request)
        {
            ...
            if (headers != null)
            {
                if (!contentType.IsNone && headers.ContainsKey("Content-Type"))
                {
                    contentType.Value = headers["Content-Type"];
                }

                if (!contentLength.IsNone && headers.ContainsKey("Content-Length"))
                {
                    contentLength.Value = headers["Content-Length"];
                }

                if (!lastModified.IsNone && headers.ContainsKey("Last-Modified"))
                {
                    lastModified.Value = headers["Last-Modified"];
                }

                if (!eTag.IsNone && headers.ContainsKey("ETag"))
                {
            ...
            {
                Debug.Log($"[API HEAD] Response {code}");
                Debug.Log($"[API HEAD] Headers: {GetResponseHeaders(request)}");
            }
        ...
        private void HandleTimeout(UnityWebRequest request)
        {
            ...
            if (logResponse.Value || debugMode.Value)
            {
                Debug.LogWarning("[API HEAD] Request timeout");
            }
            ...
            if (retryCount < maxRetries.Value)
            {
                ...
                if (debugMode.Value)
                {
                    Debug.Log($"[API HEAD] Retrying... Attempt {retryCount}/{maxRetries.Value}");
                }
        ...
        private void HandleNetworkError(UnityWebRequest request)
        {
            ...
            if (logResponse.Value || debugMode.Value)
            {
                Debug.LogError($"[API HEAD] Network Error: {request.error}");
            }
        ...
        private void HandleOtherError(UnityWebRequest request)
        {
            ...
            if (logResponse.Value || debugMode.Value)
            {
                Debug.LogError($"[API HEAD] Error: {request.error}");
            }
        ...
        {
            Debug.Log("=== API HEAD Request Details ===");
