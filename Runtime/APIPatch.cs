...
namespace HutongGames.PlayMaker.Actions
{
    ...
    [Tooltip("Performs a PATCH request to a REST API endpoint with full configuration options.")]
    public class APIPatch : FsmStateAction
    {
        ...
        [Title("Request Body")]
        [Tooltip("Request body content")]
        [UIHint(UIHint.TextArea)]
        public FsmString requestBody;

        [Tooltip("Body format type")]
        public BodyFormat bodyFormat = BodyFormat.JSON;

        [Tooltip("Content-Type header (auto-set based on format or manual)")]
        ...
        [Tooltip("JSON path to extract (e.g., data.user.name)")]
        public FsmString jsonPath;

        [Tooltip("Store parsed JSON value")]
        [UIHint(UIHint.Variable)]
        public FsmString parsedValue;
        ...
        public override void Reset()
        {
            ...
            userAgent = "Unity PlayMaker API Patch";
            timeout = 30f;
        ...
        public override void OnEnter()
        {
            ...
            retryCount = 0;
            Fsm.Owner.StartCoroutine(SendPatchRequest());
        }

        private IEnumerator SendPatchRequest()
        {
            string finalUrl = BuildUrl();

            if (logRequest.Value || debugMode.Value)
            {
                Debug.Log($"[API PATCH] Request to: {finalUrl}");
                Debug.Log($"[API PATCH] Body: {requestBody.Value}");
            }

            UnityWebRequest request = new UnityWebRequest(finalUrl, "PATCH");

            if (!string.IsNullOrEmpty(requestBody.Value))
            {
                byte[] bodyRaw = Encoding.UTF8.GetBytes(requestBody.Value);
        ...
        private void SetupHeaders(UnityWebRequest request)
        {
            ...
            {
                contentTypeValue = GetContentTypeForFormat(bodyFormat);
            }
            request.SetRequestHeader("Content-Type", contentTypeValue);
        ...
        private string GetContentTypeForFormat(BodyFormat format)
        {
            switch (format)
            {
                ...
                case BodyFormat.FormData:
                    return "application/x-www-form-urlencoded";
                case BodyFormat.RawText:
                    return "text/plain";
                default:
                    return "application/json";
            }
        ...
        private void HandleResponse(UnityWebRequest request)
        {
            ...
            {
                Debug.Log($"[API PATCH] Response {code}: {response}");
            }
        ...
        private void HandleTimeout(UnityWebRequest request)
        {
            ...
            if (logResponse.Value || debugMode.Value)
            {
                Debug.LogWarning("[API PATCH] Request timeout");
            }
            ...
            if (retryCount < maxRetries.Value)
            {
                ...
                if (debugMode.Value)
                {
                    Debug.Log($"[API PATCH] Retrying... Attempt {retryCount}/{maxRetries.Value}");
                }
        ...
        private void HandleNetworkError(UnityWebRequest request)
        {
            ...
            if (logResponse.Value || debugMode.Value)
            {
                Debug.LogError($"[API PATCH] Network Error: {request.error}");
            }
        ...
        private void HandleOtherError(UnityWebRequest request)
        {
            ...
            if (logResponse.Value || debugMode.Value)
            {
                Debug.LogError($"[API PATCH] Error: {request.error}");
            }
        ...
        private void ParseJSON(string json)
        {
            ...
            {
                Debug.LogError($"[API PATCH] JSON Parse Error: {e.Message}");
            }
        }
        ...
        private string ExtractJSONValue(string json, string path)
        {
            ...
            {
                int startIndex = current.IndexOf($"\"{key}\"");
                if (startIndex == -1) return "";

                int colonIndex = current.IndexOf(":", startIndex);
                if (colonIndex == -1) return "";

                int valueStart = colonIndex + 1;
                while (valueStart < current.Length && char.IsWhiteSpace(current[valueStart]))
                {
                    valueStart++;
                }

                if (current[valueStart] == '"')
                {
                    valueStart++;
                    int valueEnd = current.IndexOf('"', valueStart);
                    return current.Substring(valueStart, valueEnd - valueStart);
                }
                else if (current[valueStart] == '{')
                {
                    int braceCount = 1;
                    int i = valueStart + 1;
                    while (i < current.Length && braceCount > 0)
                    {
                        if (current[i] == '{') braceCount++;
                        if (current[i] == '}') braceCount--;
                        i++;
                    }
                    current = current.Substring(valueStart, i - valueStart);
                }
            }
